...
def submit():
    ...
    if form.validate_on_submit():
        ...
        # check if file is a pdf
         if (file.filename.rsplit('.', 1)[1].lower() == 'pdf') or (file.filename.rsplit('.', 1)[1].lower() == 'doc') or (file.filename.rsplit('.', 1)[1].lower() == 'docx'):
...
#df.to_csv("detail.csv",index=None)
headings=("FILEPATH","KEYWORDS","MATCHES","EXPERIENCE","MOBILE","EMAIL")
datas=(
    ("Jana_Resume_doc.pdf" ,"abinitio", "abinitio","3", "1234567890", "abc@GMAIL"),
    ("Jana_Resume_doc.pdf" ,"abinitio", "abinitio","3", "1234567890", "abc@GMAIL")
)
# page to review the result data set
@app.route('/result', methods=['GET', 'POST'])
def result():
    #data=pd.read_csv("detail.csv")
    return render_template("result.html",headings=headings,tables=datas)
    #return render_template("result.html",tables=[data.to_html()],titles=[''])
